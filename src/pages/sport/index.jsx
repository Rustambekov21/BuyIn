...
import { SportButtonList, SportList, SportMenuBrend, SportMenuPeriod, SportMenuSize } from "../../utils/pages/sport-list";
...
const SportPage = () => {
    ...
    return <div className="sport">
        <div className="sport-top">
            <p className="sport-loc">Главная  /  Спорт  /  <b>Спортивные кроссовки</b>  </p>
                ...
                <div className="order-period__list">
                    {
                        SportMenuPeriod.map(item => (
                            <Flex key={item.id}><Radio/>{item.title}</Flex>
                            ))
                        }
                </div>
            </div>
                ...
                <div className="menu-brend__list">
                    {
                        SportMenuBrend.map(item => (
                            <Flex key={item.id} gap="5px"><Checkbox />{item.title}</Flex>
                            ))
                        }
                </div>
                <div className="brend-link"><Link>Показать все</Link></div>
            </div>
        ...
        <div className="sport-main">
            {
                    SportList.map(item => ( 
                        <NavLink to={item.path} key={item.id} className="sport-link">
                        <div className="sport-card">
                            <div className="sport-img">
                                <img src={item.img} alt="" />
                            </div>
                            <h2 className="sport-card__title">{item.title}</h2>
                            ...
                            <h1 className="card-price sport-price">{item.price}</h1>
                        </div>
                    </NavLink>
                ))
                }
                <Button className="trend-btn sport-btn"><Icons.trendBtn /> Показать еще</Button>
            </div>
            </Flex>
    </div>
}
